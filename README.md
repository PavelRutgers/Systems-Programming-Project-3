...
Summary: 
This project simulates a simple UNIX shell in C. The shell reads input using the read() system call, tokenizes the input, parses it into structured commands, and executes those commands. The program supports pipelines, input/output redirection, wildcard expansion, and built-in commands such as cd, pwd, which, and exit. The shell operates in both interactive mode, which mimics a real shell terminal, and batch mode, which executes commands from a file or non-terminal input. In interactive mode, mysh prints a welcome and goodbye message, while in batch mode it does not.
...
After all features were implemented, we ran the shell under Valgrind using a mixture of built-in commands, wildcard expansion, pipelines, and redirection. The program completed execution without any memory leaks or invalid memory accesses.
